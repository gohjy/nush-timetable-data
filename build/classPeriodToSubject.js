...
async function convertFile(filepath) {
    try {
        ...
        let output = {
            ...
            subject: JSON.stringify(periodToSubject(content, { version: "3-class-subject" }))
        };
...
async function main() {
    ...
    allFiles = allFiles.filter(pathname => {
        let filetype = pathname.split(path.sep).at(-2);
        if (filetype !== "class") return false;
        return true;
    }).sort();
